...
static int f2fs_ioc_dedup_file(struct file *filp, unsigned long arg)
{
	...
	//if try to dedup an inode prepared for snapshot, clear prepare flag & dedup
	if (is_inode_flag_set(base_inode, FI_SNAPSHOT_PREPARED) &&
		!is_inode_flag_set(dedup_inode, FI_SNAPSHOTED)) {
		clear_inode_flag(base_inode, FI_SNAPSHOT_PREPARED);
		f2fs_warn(sbi, "%s: base inode[%lu] is prepared for snapshot!",
						__func__, base_inode->i_ino);
	}
