...
static bool is_libc_vma_name_eligible(struct mm_struct *mm, unsigned long addr)
{
	bool ret = false;
						 ...
						 unsigned long len)
{
	...
	switch (kname[0]) {
	case 'd':
		/* maybe a dalvik main name */
		if (len >= SZ_256M && !rsv &&
		    strcmp(kname, DALVIK_MAIN_HEAP) == 0) {
			mm->android_kabi_reserved1 = (unsigned long)uname;
			return CHP_VMA_DALVIK;
		}
		break;
	case 'l':
		/* jemalloc allocator vma name, chunk >= SZ_2M */
		if (len >= SZ_2M && strcmp(kname, NATIVE_HEAP_JEMALLOC) == 0 &&
		    is_libc_vma_name_eligible(mm, uaddr))
			return CHP_VMA_NATIVE;
		break;
	case 's':
		/* scudo allocator vma name, chunk >= SZ_64K */
		if (len >= SZ_64K &&
		    strncmp(kname, NATIVE_HEAP_SCUDO_PREFIX,
			    sizeof(NATIVE_HEAP_SCUDO_PREFIX) - 1) == 0 &&
		    is_libc_vma_name_eligible(mm, uaddr))
			return CHP_VMA_NATIVE;
		break;
	default:
